...
template class LazyLTMOA<2>;
template class LazyLTMOA<3>;
template class LazyLTMOA<4>;
template class LazyLTMOA<5>;

std::shared_ptr<AbstractSolver> get_LazyLTMOA_solver(
    AdjacencyMatrix &graph, AdjacencyMatrix &inv_graph,
    size_t start_node, size_t target_node
) {
    int num_obj = graph.get_num_obj();
    if (num_obj == 2) {
        return std::make_shared<LazyLTMOA<2>>(graph, inv_graph, start_node, target_node);
    } else if (num_obj == 3) {
        return std::make_shared<LazyLTMOA<3>>(graph, inv_graph, start_node, target_node);
    } else if (num_obj == 4) {
        return std::make_shared<LazyLTMOA<4>>(graph, inv_graph, start_node, target_node);
    } else if (num_obj == 5) {
        return std::make_shared<LazyLTMOA<5>>(graph, inv_graph, start_node, target_node);
    } else {
        return nullptr;
    }
}
