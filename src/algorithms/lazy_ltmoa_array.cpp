...
template<int N>
void LazyLTMOA_array<N>::solve(unsigned int time_limit) {
    std::cout << "start LazyLTMOA_array\n";
...
template class LazyLTMOA_array<2>;
template class LazyLTMOA_array<3>;
template class LazyLTMOA_array<4>;
template class LazyLTMOA_array<5>;
...
std::shared_ptr<AbstractSolver> get_LazyLTMOA_array_solver(
    ...
    if (num_obj == 2) {
        return std::make_shared<LazyLTMOA_array<2>>(graph, inv_graph, start_node, target_node);
    } else if (num_obj == 3) {
        return std::make_shared<LazyLTMOA_array<3>>(graph, inv_graph, start_node, target_node);
    } else if (num_obj == 4) {
        return std::make_shared<LazyLTMOA_array<4>>(graph, inv_graph, start_node, target_node);
    } else if (num_obj == 5) {
        return std::make_shared<LazyLTMOA_array<5>>(graph, inv_graph, start_node, target_node);
    } else {
        return nullptr;
    }
}
